 ...
 * 
 * @author Not fully determined but assmed to be among Marie Van Den Bogaard,
 *         Léo Exibard, Gilles Geeraerts. Javadoc by Mathieu Sassolas.
 */
public class ParseException extends Exception {
     ...
     * 
     * @return a String containing the list of expected terminals (with English
     *         fillers).
     */
    ...
    public String getMessage() {
        if (variable == null) {
            return String.format("Parsing Error at line %d and column %d: %s%s", token.getLine(), token.getColumn(),
                    stringOfAlternatives(), token.getValue());
        } else {
            return String.format("Parsing Error at line %d and column %d trying to parse %s: %s%s", token.getLine(),
                    token.getColumn(), variable.toString(), stringOfAlternatives(), token.getValue().toString());
        }
    }
}
